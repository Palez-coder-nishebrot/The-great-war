			...
			for need_good in pop_unit.population_type.needs:
				...
				if market_good_quantity > needed_good_quantity or is_equal_approx(market_good_quantity, needed_good_quantity):
					if pop_unit.money >= price:
						...
						pop_unit.expenses += Functions.buy_good_on_local_market(pop_unit, good, needed_good_quantity, client.economy_manager)
						pop_unit.welfare += 1
				else:
					var im_good_quantity = needed_good_quantity - market_good_quantity
					var new_price = Functions.get_good_price(price, im_good_quantity, market_good_quantity, client.economy_manager.tariffs)
					...
					if money > new_price or is_equal_approx(money, new_price):
						demand_supply_goods[good][0] += needed_good_quantity
						if is_buying_valid:
							GlobalMarket.fill_lack(client.economy_manager, good, im_good_quantity)
							pop_unit.expenses += Functions.buy_good_on_local_market(pop_unit, good, needed_good_quantity, client.economy_manager, im_good_quantity)
							pop_unit.welfare += 1
