...
func load_list():
	...
	var list_of_tiles = get_parent().get_node("TileMap").list_of_tiles
	...
	for i in get_parent().get_node("TileMap").list_of_tiles:
		...
		if list_of_resources_of_provinces.has(i):
			tile.resources = list_of_resources_of_provinces[i]
		
		if list_of_factories_of_provinces.has(i) and not tile.get_groups().has("Village"):
			tile.list_of_buildings = list_of_resources_of_provinces[i]
